...
pub(crate) use struct_meta::StructMetaInfo;
use struct_meta::StructField;

use syn::{Ident, Generics, DeriveInput, Data, Fields, Meta, spanned::Spanned, Path};

use crate::{field_attributes::{self}, trait_attributes::ReflectTraits, REFLECT_ATTR, REFLECT_VALUE_ATTR, crate_manifest::CrateManifest};
...
/// Metadata on all reflected types.
...
impl<'a> ReflectTypeMetaInfo<'a> {
    pub fn from_derive_input(input: &'a DeriveInput) -> anyhow::Result<Self, syn::Error> {
        ...
        for meta in input.attrs.iter().filter_map(|attr| attr.parse_meta().ok()) {
            match meta {
                ...
                Meta::List(meta_list) if meta_list.path.is_ident(REFLECT_ATTR) => {
                    if !matches!(reflect_mode, None | Some(ReflectMode::Common)) {
                        return Err(syn::Error::new(
                            meta_list.span(),
                            format_args!("Cannot use both `#[{REFLECT_ATTR}]` and `#[{REFLECT_VALUE_ATTR}]`"),
                        ));
                    }
                ...
                Meta::List(meta_list) if meta_list.path.is_ident(REFLECT_VALUE_ATTR) => {
                ...
                Meta::Path(meta_path) if meta_path.is_ident(REFLECT_VALUE_ATTR) => {
                    if !matches!(reflect_mode, None | Some(ReflectMode::Primitive)) {
                        return Err(syn::Error::new(
                            meta_path.span(),
                            format_args!("cannot use both `#[{REFLECT_ATTR}]` and `#[{REFLECT_VALUE_ATTR}]`"),
        ...
        match &input.data {
            Data::Struct(data) => {
                ...
                match data.fields {
                    Fields::Named(_) => Ok(Self::Struct(struct_meta)),
                    _ => unimplemented!()
                }
            }
            _ => unimplemented!()
    ...
    fn collect_struct_fields(fields: &Fields) -> anyhow::Result<Vec<StructField>, syn::Error> {
        let struct_fields: anyhow::Result<Vec<StructField>, syn::Error> = fields.iter().enumerate()
            .map(|(index, field)| -> anyhow::Result<StructField, syn::Error> {
