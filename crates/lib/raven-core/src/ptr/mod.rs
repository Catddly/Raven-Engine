...
pub struct SingletonRefPtr<T: 'static>(*mut T, PhantomData<T>);

unsafe impl<T: Send> Send for SingletonRefPtr<T> {}
unsafe impl<T: Sync> Sync for SingletonRefPtr<T> {}

impl<T: 'static> SingletonRefPtr<T> {
    ...
    pub fn new(singleton: &mut T) -> Self {
        Self(
            singleton,
            PhantomData,
        )
    ...
    pub fn get_ref(&self) -> &T {
        unsafe { self.0.as_ref() }.expect("Try to get reference from a null pointer!")
    }

    pub fn replace(&mut self, singleton: &mut T) {
        self.0 = singleton;
    }

    pub fn invalidata(mut self) {
